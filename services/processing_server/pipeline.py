...
# When set, ML classification is delegated to this service instead of local models.
...
def _ml_service_classify(image_bytes: bytes) -> Tuple[str, float]:
    """Call the external ml-service /predict endpoint."""
    resp = requests.post(
    ...
    return model.get("class_name", "others"), float(model.get("probability", 0.0))


def _ml_segmentation_and_classification(image_bytes: bytes, req: AnalysisRequest) -> Tuple[Metrics, str]:
    # ── Bridge to external ml-service ───────────────────────────
    if ML_SERVICE_URL:
        try:
            detected_condition, confidence = _ml_service_classify(image_bytes)
            logger.info("ml-service: condition=%s confidence=%.2f", detected_condition, confidence)
            return Metrics(
                lesion_count=0,
                severity_score=round(confidence, 2) if detected_condition != "others" else 0.0,
                per_region={"forehead": 0, "left_cheek": 0, "right_cheek": 0},
                extra={
                    "problem_type": detected_condition,
                    "is_follow_up": getattr(req, "is_follow_up", False),
                    "detector": "ml_service",
                    "confidence": confidence,
                },
            ), detected_condition
        except Exception as e:
            logger.warning("ml-service call failed (%s), trying local models", e)
...
def run_analysis_pipeline(
    ...
    def report(step: str) -> None:
        logger.info("progress step=%s job=%s", step, req.request_id)
